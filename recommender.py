...
def model_train():
  ...
  pickle.dump(als_model, open(saved_model_fname, 'wb')) # wb : write binary, 모델을 저장하는 함수
  return als_model
...
def calculate_user_based(user_items, items):
  ...
  # 사용자가 평가한 데이터를 기반으로 해당 사용자를 모델에 추가하는 방식으로 동작ㅍ
  ...
  return [str(items[r]) for r in recs[0]] # 추천 결과를 아이템 ID에서 아이템 이름으로 변환하여 반환
...
if __name__ == "__main__":
    command = sys.argv[1]  # 첫 번째 인자로 명령어를 받음

    if command == "item-based":
        item_id = sys.argv[2]  # 두 번째 인자로 아이템 ID
        print(json.dumps(item_based_recommendation(item_id)))  # 결과를 JSON으로 출력

    elif command == "user-based":
        try:
            input_data = sys.stdin.read()  # 사용자가 평가한 아이템 ID와 평점 정보를 JSON 형식으로 받음
            input_rating_dict = json.loads(input_data)  # JSON 형식을 딕셔너리로 변환
            result_items = user_based_recommendation(input_rating_dict)  # 사용자 기반 추천 함수 호출
            print(json.dumps(result_items))  # 결과를 JSON 형식으로 출력
        except Exception as e:
            print(f"Error processing user-based recommendation: {e}", file=sys.stderr)
    else:
        print('Error: Invalid command')
        sys.exit(1)

  # model_train() # 모델 학습 함수 호출, 데이터를 학습하여 모델을 생성하는 함수, 처음 한번 실행 후 주석 처리. 이후 데이터가 추가됐을 때 주기적으로 실행하여 모델을 업데이트

  # input_rating_dict = {1: 4, 2: 3.5} # 사용자가 평가한 아이템 ID와 평점 정보를 담은 딕셔너리

  # result_items = user_based_recommendation(input_rating_dict) # 사용자 기반 추천 함수 호출, 사용자 기반 협업 필터링을 사용하여 추천을 제공하는 함수
